  ...
  services = {
    thinkfan = {
      ...
      levels = [
        ...
        [0 0  50]
        ["level auto" 45 75]
        # ИСПРАВЛЕНО: было ["level disengaged" 90 32767] — между upper=80
        # уровня 7 и lower=90 этого уровня оставался незакрытый диапазон
        # 80-90°C, thinkfan в simple-синтаксисе требует непрерывного
        # покрытия (upper предыдущего уровня должен касаться/перекрывать
        # lower следующего, как и во всех предыдущих парах этого списка).
        # Именно этот разрыв ронял сервис при старте:
        #   ERROR: LOWER limit doesn't overlap with previous UPPER limit
        # lower = 80 продолжает тот же паттерн касания границ, что и выше.
        ["level disengaged" 70 32767]
      ];
    };
