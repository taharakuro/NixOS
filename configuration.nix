  ...
  services = {
    thinkfan = {
      ...
      levels = [
        ...
        #
        # ИСПРАВЛЕНО: границы раньше касались впритык ([0,0,45] → [1,45,50]
        # — up и down происходили на одном и том же градусе), поэтому при
        # температуре, зависшей около границы (типично около 45° на
        # холостом ходу), вентилятор непрерывно щёлкал вкл/выкл. Официальный
        # пример конфига thinkfan.conf(5) как раз показывает нужный паттерн:
        # [0,0,50] → ["level auto",45,75] — lower следующего уровня (45)
        # СТРОГО МЕНЬШЕ upper предыдущего (50). Пока температура внутри
        # этого перекрытия, thinkfan держит текущий уровень и не дёргается.
        # Взял стандартный запас 5° на каждой границе.
        [0 0  48]
        [1 43 53]
        [2 48 58]
        [3 53 63]
        ...
        [6 68 78]
        [7 73 83]
        # lower = 78 < upper(level 7) = 83 — тот же паттерн перекрытия, без
        # него получим ту же ошибку старта, что была с разрывом 80→90:
        #   ERROR: LOWER limit doesn't overlap with previous UPPER limit
