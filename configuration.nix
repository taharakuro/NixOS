...

{
  ...
  services = {
    thinkfan = {
      ...
      levels = [
        # Перекрытие ~5° на каждой границе: lower(N) < upper(N-1),
        # как в примере thinkfan.conf(5): [0,0,50] → ["level auto",45,75].
        # Раньше границы просто соприкасались (40→40, 45→45, ...) —
        # это НЕ перекрытие, а стык встык, и именно это вызывает дребезг
        # на пороговой температуре, который комментарий выше описывал
        # как уже пофикшенный баг.
        [0 0  40]
        [1 35 45]
        [2 40 50]
        [3 45 55]
